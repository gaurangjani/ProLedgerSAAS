...
(function () {
  ...
  window.LoginModal = function ({ onSuccess }) {
    ...
    const submit = async () => {
      ...
      try {
        let res;
        if (mode === 'login') {
          res = await window.API.auth.login({ email: form.email, password: form.password });
        } else if (mode === 'register') {
          if (!form.orgName) { setError('Organisation name is required'); setLoading(false); return; }
          res = await window.API.auth.register({ name: form.name, email: form.email, password: form.password, orgName: form.orgName });
        } else {
          res = await window.API.auth.acceptInvite({ token: form.token, name: form.name, password: form.password });
        }
        window.API.currentUser = res.data.user;
        window.API.currentOrg  = res.data.org;
