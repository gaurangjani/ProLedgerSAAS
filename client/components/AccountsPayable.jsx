...
(function () {
  ...
  const PO_STATUSES = ['draft','sent','acknowledged','partially-received','received','cancelled'];
  ...
  window.APModule = function () {
    ...
    const loaders = {
      vendors:  async () => { const r = await window.API.ap.getVendors();        setVendors(r.data||[]); },
      bills:    async () => { const r = await window.API.ap.getBills();          setBills(r.data||[]); },
      po:       async () => { const r = await window.API.ap.getPurchaseOrders(); setPos(r.data||[]); },
      payments: async () => { const r = await window.API.ap.getPayments();       setPayments(r.data||[]); }
    };
    ...
    const saveVendor = async (e) => {
      ...
      try {
        if (editItem) await window.API.ap.updateVendor(editItem._id, formData);
        else await window.API.ap.createVendor(formData);
        closeModal(); load();
      } catch (err) { setFormError(err.message); }
    };

    const deleteVendor = async () => {
      try { await window.API.ap.deleteVendor(confirmTarget._id); setConfirmTarget(null); load(); }
    ...
    const saveBill = async (e) => {
      e.preventDefault(); setFormError(null);
      try {
        await window.API.ap.createBill({...billForm, lineItems: billForm.lineItems.map(l=>({...l,quantity:parseFloat(l.quantity)||1,unitPrice:parseFloat(l.unitPrice)||0,taxRate:parseFloat(l.taxRate)||0}))});
        closeModal(); load();
      } catch (err) { setFormError(err.message); }
    };
    ...
    const savePO = async (e) => {
      e.preventDefault(); setFormError(null);
      try {
        await window.API.ap.createPurchaseOrder({...poForm, lineItems: poForm.lineItems.map(l=>({...l,quantity:parseFloat(l.quantity)||1,unitPrice:parseFloat(l.unitPrice)||0}))});
        closeModal(); load();
      } catch (err) { setFormError(err.message); }
    };
    ...
    const savePayment = async (e) => {
      e.preventDefault(); setFormError(null);
      try {
        await window.API.ap.createPayment({...paymentForm, amount:parseFloat(paymentForm.amount)||0});
        closeModal(); load();
      } catch (err) { setFormError(err.message); }
    };
    ...
    return (
      <div className="module-container">
        ...
        {activeTab === 'vendors' && (<>
          ...
          <window.DataTable loading={loading} emptyText="No vendors yet."
            columns={[
        ...
        {activeTab === 'bills' && (<>
          ...
          {error && <window.ErrorBanner message={error} onRetry={load} />}
          <window.DataTable loading={loading} emptyText="No bills yet."
            columns={[
              {key:'billNumber',label:'Bill #'},{key:'vendor',label:'Vendor',render:(v,r)=>r.vendor?.vendorName||'—'},
              {key:'billDate',label:'Date',render:v=>window.formatDate(v)},{key:'dueDate',label:'Due',render:v=>window.formatDate(v)},
              {key:'total',label:'Total',render:v=>window.formatCurrency(v)},{key:'amountDue',label:'Outstanding',render:v=>window.formatCurrency(v)},
        ...
        {activeTab === 'po' && (<>
          ...
          {error && <window.ErrorBanner message={error} onRetry={load} />}
          <window.DataTable loading={loading} emptyText="No purchase orders yet."
            columns={[
              {key:'poNumber',label:'PO #'},{key:'vendor',label:'Vendor',render:(v,r)=>r.vendor?.vendorName||'—'},
              {key:'orderDate',label:'Order Date',render:v=>window.formatDate(v)},
              {key:'expectedDeliveryDate',label:'Expected Delivery',render:v=>window.formatDate(v)},
        ...
        {activeTab === 'payments' && (<>
          ...
          {error && <window.ErrorBanner message={error} onRetry={load} />}
          <window.DataTable loading={loading} emptyText="No vendor payments yet."
            columns={[
              ...
              {key:'paymentMethod',label:'Method'},{key:'status',label:'Status',render:v=><window.StatusBadge status={v}/>}
            ]}
            data={payments}
          />
        </>)}
        ...
        {showModal && activeTab==='vendors' && (
          <window.Modal title={editItem?'Edit Vendor':'Add Vendor'} onClose={closeModal} footer={<><button className="btn-secondary" onClick={closeModal}>Cancel</button><button className="btn-primary" onClick={saveVendor}>Save</button></>}>
            {formError && <window.ErrorBanner message={formError}/>}
            <form onSubmit={saveVendor}>
              <div className="form-row">
                <window.FormField label="Vendor Code *"><input className="form-input" value={formData.vendorCode||''} onChange={e=>setFormData(f=>({...f,vendorCode:e.target.value}))} required /></window.FormField>
                <window.FormField label="Vendor Name *"><input className="form-input" value={formData.vendorName||''} onChange={e=>setFormData(f=>({...f,vendorName:e.target.value}))} required /></window.FormField>
              </div>
              <div className="form-row">
              ...
              <div className="form-row">
                ...
                <window.FormField label="Tax ID"><input className="form-input" value={formData.taxId||''} onChange={e=>setFormData(f=>({...f,taxId:e.target.value}))} /></window.FormField>
              </div>
        ...
        {showModal && activeTab==='bills' && (
          <window.Modal title="New Bill" onClose={closeModal} footer={<><button className="btn-secondary" onClick={closeModal}>Cancel</button><button className="btn-primary" onClick={saveBill}>Create Bill</button></>}>
            {formError && <window.ErrorBanner message={formError}/>}
            <div className="form-row">
              <window.FormField label="Vendor *">
              ...
              <window.FormField label="Bill Date *"><input className="form-input" type="date" value={billForm.billDate} onChange={e=>setBillForm(f=>({...f,billDate:e.target.value}))} required /></window.FormField>
            </div>
            <window.FormField label="Notes"><input className="form-input" value={billForm.notes||''} onChange={e=>setBillForm(f=>({...f,notes:e.target.value}))} /></window.FormField>
            ...
            <div style={{display:'flex',justifyContent:'space-between',marginTop:8}}>
              ...
              <div className="line-items-totals">Subtotal: {window.formatCurrency(billSubtotal)} | Tax: {window.formatCurrency(billTax)} | <strong>Total: {window.formatCurrency(billSubtotal+billTax)}</strong></div>
            </div>
          </window.Modal>
        )}
        ...
        {showModal && activeTab==='po' && (
          <window.Modal title="New Purchase Order" onClose={closeModal} footer={<><button className="btn-secondary" onClick={closeModal}>Cancel</button><button className="btn-primary" onClick={savePO}>Create PO</button></>}>
            {formError && <window.ErrorBanner message={formError}/>}
            <div className="form-row">
              <window.FormField label="Vendor *">
              ...
              <window.FormField label="Order Date *"><input className="form-input" type="date" value={poForm.orderDate} onChange={e=>setPoForm(f=>({...f,orderDate:e.target.value}))} required /></window.FormField>
            </div>
            <window.FormField label="Expected Delivery"><input className="form-input" type="date" value={poForm.expectedDeliveryDate||''} onChange={e=>setPoForm(f=>({...f,expectedDeliveryDate:e.target.value}))} /></window.FormField>
            <table className="line-items-table">
            ...
            <div style={{display:'flex',justifyContent:'space-between',marginTop:8}}>
              ...
              <div className="line-items-totals"><strong>Total: {window.formatCurrency(poTotal)}</strong></div>
            </div>
          </window.Modal>
        )}
        ...
        {showModal && activeTab==='payments' && (
          <window.Modal title="Record Vendor Payment" onClose={closeModal} footer={<><button className="btn-secondary" onClick={closeModal}>Cancel</button><button className="btn-primary" onClick={savePayment}>Record</button></>}>
            {formError && <window.ErrorBanner message={formError}/>}
            <div className="form-row">
              <window.FormField label="Bill *">
                <select className="form-select" value={paymentForm.bill} onChange={e=>{const b=bills.find(x=>x._id===e.target.value);setPaymentForm(f=>({...f,bill:e.target.value,vendor:b?.vendor?._id||b?.vendor||'',amount:b?.amountDue||0}));}} required>
                  ...
                  {bills.filter(b=>['pending','partially-paid','overdue'].includes(b.status)).map(b=><option key={b._id} value={b._id}>{b.billNumber} — {window.formatCurrency(b.amountDue)} due</option>)}
                </select>
              </window.FormField>
        ...
        {confirmTarget && <window.ConfirmDialog message={`Delete vendor "${confirmTarget.vendorName}"?`} onConfirm={deleteVendor} onCancel={()=>setConfirmTarget(null)}/>}
      </div>
    );
  };
