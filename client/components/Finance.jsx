...
(function () {
  ...
  window.FinanceModule = function () {
    ...
    const loadAccounts = async () => {
      ...
      try { const r = await window.API.finance.getAccounts(); setAccounts(r.data || []); }
    ...
    const loadEntries = async () => {
      setLoading(true); setError(null);
      try { const r = await window.API.finance.getJournalEntries(); setEntries(r.data || []); }
      catch (e) { setError(e.message); }
      finally { setLoading(false); }
    };
    ...
    const loadTrialBalance = async () => {
      setLoading(true); setError(null);
      try { const r = await window.API.finance.getTrialBalance(); setTrialBalance(r.data || r); }
      catch (e) { setError(e.message); }
      finally { setLoading(false); }
    };

    useEffect(() => {
      checkAuth().then(ok => { if (ok) { if (activeTab === 'accounts') loadAccounts(); else if (activeTab === 'journal') loadEntries(); else if (activeTab === 'trial') loadTrialBalance(); } });
    }, [activeTab]);
    ...
    const saveAccount = async (e) => {
      ...
      try {
        if (editItem) await window.API.finance.updateAccount(editItem._id, formData);
        else await window.API.finance.createAccount(formData);
        closeModal(); loadAccounts();
      } catch (err) { setFormError(err.message); }
    };

    const deleteAccount = async () => {
      try { await window.API.finance.deleteAccount(confirmTarget._id); setConfirmTarget(null); loadAccounts(); }
    ...
    const saveEntry = async (e) => {
      ...
      try {
        await window.API.finance.createJournalEntry({ ...entryForm, lines: entryForm.lines.map(l => ({ ...l, debit: parseFloat(l.debit) || 0, credit: parseFloat(l.credit) || 0 })) });
    ...
    const AccountsTab = () => (
      <>
        ...
        {error && <window.ErrorBanner message={error} onRetry={loadAccounts} />}
        <window.DataTable
          columns={[
            { key: 'accountCode', label: 'Code' },
            { key: 'accountName', label: 'Name' },
            { key: 'accountType', label: 'Type', render: v => <window.StatusBadge status={v} /> },
            { key: 'accountCategory', label: 'Category' },
            { key: 'balance', label: 'Balance', render: v => window.formatCurrency(v) },
    ...
    const JournalTab = () => (
      <>
        ...
        {error && <window.ErrorBanner message={error} onRetry={loadEntries} />}
        <window.DataTable
          columns={[
            { key: 'entryNumber', label: 'Entry #' },
            { key: 'date', label: 'Date', render: v => window.formatDate(v) },
            { key: 'description', label: 'Description' },
            { key: 'type', label: 'Type' },
            { key: 'totalDebit', label: 'Total Debit', render: v => window.formatCurrency(v) },
          ...
          actions={row => row.status === 'draft' ? (
            <button className="btn-secondary btn-sm" onClick={async () => { try { await window.API.finance.postJournalEntry(row._id); loadEntries(); } catch (e) { setError(e.message); } }}>Post</button>
          ) : null}
        />
      </>
    );
    ...
    const TrialTab = () => (
      <>
        ...
        {error && <window.ErrorBanner message={error} onRetry={loadTrialBalance} />}
        {loading && <window.LoadingSpinner text="Generating trial balance..." />}
              ...
              <tbody>
                {(trialBalance.accounts || []).map((a, i) => (
                  <tr key={i}>
                    ...
                    <td>{a.accountName}</td>
                    <td>{a.accountType}</td>
                    <td>{a.debit > 0 ? window.formatCurrency(a.debit) : '—'}</td>
                    <td>{a.credit > 0 ? window.formatCurrency(a.credit) : '—'}</td>
                  </tr>
                ))}
    ...
    return (
      <div className="module-container">
        ...
        {showModal && activeTab !== 'journal' && (
          <window.Modal title={editItem ? 'Edit Account' : 'Add Account'} onClose={closeModal} footer={<>
            ...
            <form id="account-form" onSubmit={saveAccount}>
              ...
              <window.FormField label="Description">
        ...
        {/* Journal Entry modal */}
        {showModal && activeTab === 'journal' && (
          <window.Modal title="New Journal Entry" onClose={closeModal} footer={<>
            <button className="btn-secondary" onClick={closeModal}>Cancel</button>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 8 }}>
              ...
              <div className="line-items-totals">
                Debit: <strong style={{ color: Math.abs(totalDebit - totalCredit) < 0.01 ? '#27ae60' : '#e74c3c' }}>{window.formatCurrency(totalDebit)}</strong>
                &nbsp;|&nbsp;
                Credit: <strong>{window.formatCurrency(totalCredit)}</strong>
              </div>
            </div>
          </window.Modal>
        )}

        {confirmTarget && <window.ConfirmDialog message={`Delete account "${confirmTarget.accountName}"?`} onConfirm={deleteAccount} onCancel={() => setConfirmTarget(null)} />}
      </div>
    );
  };
