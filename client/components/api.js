...
(function () {
  ...
  async function request(method, path, body) {
    ...
    try {
      ...
      if (window.API?.currentOrg?._id) opts.headers['x-org-id'] = window.API.currentOrg._id;
  ...
  window.API = {
    ...
    finance: {
      ...
      getJournals:    (p) => request('GET', '/finance/journal-entries' + q(p)),
      createJournal:  (d) => request('POST', '/finance/journal-entries', d),
      postJournal:    (id,d) => request('PUT', `/finance/journal-entries/${id}`, d),
    ...
    ap: {
      ...
      getPOs:        (p) => request('GET', '/ap/purchase-orders' + q(p)),
      createPO:       (d) => request('POST', '/ap/purchase-orders', d),
    ...
    expenses: {
      ...
      submit:          (id)   => request('POST', `/expenses/claims/${id}/submit`),
      approve:         (id)   => request('POST', `/expenses/claims/${id}/approve`),
      reject:          (id,d) => request('POST', `/expenses/claims/${id}/reject`, d),
      reimburse:       (id)   => request('POST', `/expenses/claims/${id}/reimburse`)
    },
